...
import { Tooltip as TooltipType } from 'global'
...
const Tooltip = () => {
	...
	return (
		<motion.div className="absolute bottom-20 md:bottom-4 flex flex-col items-center w-full z-20 bg" layout>
			<AnimatePresence>
				{tooltips.map((tooltip: TooltipType, i: number) => (
					<motion.div
						initial={{ opacity: 0 }}
						animate={{ opacity: 1 }}
						exit={{ opacity: 0 }}
						layout
						key={i}
						className={`flex flex-col items-center text-center max-w-xs mt-2 py-2 px-4 rounded-lg cursor-pointer ${
